<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateLidCity extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {

        Schema::table('leads', function($table)
        {
            $table->dropColumn('id');
        });
        Schema::table('orders', function($table)
        {
            $table->dropColumn('id');
        });
        Schema::table('leads', function($table)
        {
            $table->increments('id')->first();
        });
        Schema::table('orders', function($table)
        {
            $table->increments('id')->first();
        });
        Schema::create('lid_city', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('id_lid');
            $table->unsignedInteger('id_city');
        });
        Schema::create('order_city', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('id_order');
            $table->unsignedInteger('id_city');
        });
        Schema::table('lid_city', function (Blueprint $table) {
            $table->foreign('id_lid')->references('id')->on('leads');
            $table->foreign('id_city')->references('id')->on('cities');
        });
        Schema::table('order_city', function (Blueprint $table) {
            $table->foreign('id_order')->references('id')->on('orders');
            $table->foreign('id_city')->references('id')->on('cities');
        });

    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::table('lid_city', function (Blueprint $table) {
            $table->dropForeign(['id_lid']);
            $table->dropForeign(['id_city']);
        });
        Schema::table('order_city', function (Blueprint $table) {
            $table->dropForeign(['id_order']);
            $table->dropForeign(['id_city']);
        });

        Schema::dropIfExists('lid_city');
        Schema::dropIfExists('order_city');
    }
}
